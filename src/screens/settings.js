 ...
 * 직원 계정 관리 → QR 메뉴판 보기 → 자동 수락 → 매장 정보 → 로그아웃.
 * '자동 수락'은 다른 화면에 묻혀있던 토글이 아니라 이 화면 리스트에 눈에 보이는 독립 항목으로
 * 승격되어 있다 — 리스트 행 그대로에 토글을 붙인 형태(`.settings-list-item`에서 트레일링 요소만
 * chevron 대신 토글로 바꿈, 행 전체를 눌러도 토글되도록 클릭 핸들러를 행에 붙였다).
 * (재고 소진 시 자동 품절 여부는 기능명세서 기준 매장 전체 공통 설정이 아니라 메뉴 항목별
 * 필드다 — 메뉴 추가 및 수정 화면(menu.js)에 있다.)
...
(function () {
  var STATUS_META = {
    CLOSED: { label: '마감', emoji: '🔴', desc: '새 주문을 받지 않고 있어요' },
    OPEN: { label: '영업 중', emoji: '🟢', desc: '고객이 QR/태블릿으로 주문할 수 있어요' },
    PAUSED: { label: '일시중지', emoji: '🟠', desc: '일시적으로 새 주문을 받지 않고 있어요' },
  };
        ...
        '<div class="screen-scroll" style="padding: 8px 20px 24px;">' +
          '<div class="card operating-status-card" id="operating-status-card">' +
            '<div class="operating-status-header">' +
              '<span id="operating-status-emoji"></span>' +
          ...
          '<div class="settings-list-item" data-action="toggle-auto-accept-row">' +
            ...
            UI.toggle(true, 'toggle-auto-accept') +
          '</div>' +
  ...
  function mount(root) {
    ...
    function renderOperatingStatus(status) {
      ...
      root.querySelector('#operating-status-emoji').textContent = meta.emoji;
